...
namespace Gageas.モーラその後に
{
    class ID3V2Tag
    {
        ...
        /* フレームのタイプ */
        public enum FRAME_TYPE { FR_TXT, FR_TXT_PURE, FR_TXT_EX, FR_TXT_EX_LNG, FR_BIN, FR_BIN_EX, FR_APIC, FR_UNKNOWN };
	...
	new _frame_id("","","","?","",FRAME_TYPE.FR_UNKNOWN,ALL_VER), /* どれでもない */
...
//	new _frame_id("ETC","ETCO","ETCO","イベントタイムコード",FRAME_TYPE.FR_BIN,ALL_VER),
	new _frame_id("GEO","GEOB","GEOB","GEOB","パッケージ化された一般的なオブジェクト",FRAME_TYPE.FR_BIN,ALL_VER),
...
//            public System.Drawing.Image imagebody;
            ...
            public string geob_mime;
            public string geob_filename;
            public string geob_contentDesc;
            public byte[] geob_binarydata;
        };
...
static class ByteArrayUtil
{
    ...
    public static int IndexOf2B(this byte[] buffer, int offset, byte search, byte search2)
    {
        for (int i = offset; i < buffer.Length -1; i++)
        {
            if ((buffer[i] == search) && (buffer[i+1] == search2)) return i;
        }
        return -1;
    }
}